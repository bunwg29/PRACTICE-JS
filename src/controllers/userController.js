...
import axios from '@/api/apiDataConfig';
...
export default class UserController {
  ...
  async updatePaymentStatus(userId) {
    try {
      ...
      if (
        user &&
        (user.paid_status === 'Unpaid' || user.paid_status === 'Overdue')
      ) {
  ...
  async updateView() {
    ...
    this.view.pagination.totalPages = Math.ceil(
      this.view.pagination.totalItems / this.view.pagination.itemsPerPage,
    );
